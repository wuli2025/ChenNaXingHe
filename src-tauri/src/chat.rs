...
pub async fn chat_send(app: AppHandle, args: ChatSendArgs) -> Result<String, String> {
    ...
    // ───────────────────────────────────────────────────────────────────────
    // 【精简版提示词流水线 / 2026-06-29 按主人要求】对话注入只保留三类：
    //   ① Skill 提示词(显式点选 + 按意图自动激活)—— 已在上面拼入 skill_section
    //   ② 知识库注入 —— 下面的 CLAUDE.md 人格/wiki + KB 概览 + 双库强制召回 + 回声层记忆地图
    //   ③ 对话历史 + 用户问题
    // 其余注入一律**不再进对话热路径**：回答风格 / 输出·可运行项目·长任务·脚本·检索·下载公约 /
    // 分批 / 目标 / 动态编排 / 专家团智能匹配 / 生图能力检测。这些 directive 函数体仍保留并标
    // #[allow(dead_code)]，便于将来按需重新启用；删的是「自动往每轮对话里塞」，不是删功能。
    // 模型选择由前端/provider 决定，与提示词注入无关，保持不动。
    // ───────────────────────────────────────────────────────────────────────

    // 生图能力提示随上述公约一并下线：image_notice 恒为 None(下游 `if let Some(notice)` 自然空转)。
    let image_notice: Option<String> = None;
    ...
    // 3.5 跨对话产物地图已下线(非 Skill / 非 KB / 非历史，属精简掉的注入)。
    //     project_artifacts_block 保留并标 dead_code，将来需要时再接回。
...
// Docker-in-Docker 沙箱(polaris_sandbox crate)随产品改版整体下线;原 spawn_in_sandbox
// 死代码一并移除。需要时改走桌面默认的本地 spawn 路径。
...
/// 跨对话产物地图预算(字符)。精简流水线后该注入已下线，保留常量供将来接回。
#[allow(dead_code)]
...
#[allow(dead_code)] // 精简流水线后不再自动注入；保留供将来按需接回
...
/// 会渗进幻灯片/网页文案, 把成品写干瘪)。
#[allow(dead_code)] // 精简流水线后不再注入；保留供将来按需接回
