...
## 第二段 · 调壹伴脚本：两段直送草稿（v5）
脚本随包落盘在 `~/ChenNaXingHe/skills/wechat-md-typesetter/scripts/wechat_yiban.py`。
...
1.（首次）装 CloakBrowser：`pip install ~/ChenNaXingHe/plugins/cloakbrowser`（在线则 `pip install cloakbrowser`）。

2. **先预览**（确定性、不碰后台）——让用户先眼检排版：
   ```bash
   python ~/ChenNaXingHe/skills/wechat-md-typesetter/scripts/wechat_yiban.py \
...
6. **可视化排版面板**（壹伴插件形态，用户要"自己看着改/挑模板/AI 改风格"时用这个）：
   ```bash
   python ~/ChenNaXingHe/skills/wechat-md-typesetter/scripts/wechat_yiban.py --mode panel
   ```
