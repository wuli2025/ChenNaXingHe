...
1. 安装（首次运行会自动下载隐身 Chromium 二进制 ~200MB，本地缓存）：
   ...
   - 离线（Polaris 已附带源码副本）：`pip install ~/ChenNaXingHe/plugins/cloakbrowser`
...
## 随时移除
本插件可随时拿掉：在「技能中心」把「CloakBrowser 浏览器」开关关掉即可，Agent 会自动回退到普通浏览方式；删除 `~/ChenNaXingHe/plugins/cloakbrowser` 可一并清除离线源码副本。
