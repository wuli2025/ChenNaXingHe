...
export interface AutomationFlow {
  ...
  lastConvId?: string;
  /** 显式「首次见到即补跑」开关。默认(undefined/false)：首次只登记基线、不补跑，
   *  避免开自动化页时凭空触发一次 daily 流程；置 true 才在首次到点补跑。 */
...
// v4 增补「微信聊天 · 每日待办」一条（每日定时、对接 wechat-tasks 技能）：对老用户也补一次，删除后不回种
const SEED_V4_KEY = "polaris:automation-flows-seeded:v4";
...
// ── v4：微信聊天「每日待办」——本地解密微信，挖出「该回谁」，写进晨报卡片 ──
// 与撰稿类流程不同：这条是**每日定时**跑、对接本机 wechat-tasks 技能的运维型流程。
// 提示词只需驱动 agent 跑技能脚本（脚本自己解密→挖待办→写 kb_root/memory/briefing），
// 待办随后以晨报卡片出现在对话框/自动化页顶部。首次需用户做一次性授权（见技能说明）。
const wechatTasksPrompt = `你要执行「微信聊天 · 每日待办」自动化：本地解密我的微信、挖出**最近几天该回谁**、把待办写进我的晨报卡片。全程在本机完成，**不要联网、不要上传、不要对外发任何消息**。

请按以下步骤做：
1. 找到本机「微信每日待办」技能脚本目录：\`~/ChenNaXingHe/skills/wechat-tasks/scripts/\`（Windows 即 \`C:\\Users\\<我>\\ChenNaXingHe\\skills\\wechat-tasks\\scripts\\\`）。
2. 先确认 \`wx_config.json\` 存在且填了 \`master_key\`：
   - 若**不存在或 master_key 为空** → 说明还没做一次性授权。**不要**擅自重启微信，而是停下来提示我：「请先在技能里跑一次 \`python wx_setup.py\` 完成授权（会重启微信让我扫码登录），之后每天就能自动跑」，然后结束本次运行。
   - 若已配置好 → 进入下一步。
3. 运行 \`python ~/ChenNaXingHe/skills/wechat-tasks/scripts/wx_daily.py\`（用系统 python 即可，脚本内部会调微信解密 venv）。它会：复用缓存密钥解密导出当前微信库 → 从「我回过话的私聊 + 我活跃的群里、最近 7 天别人发来我还没回」的消息挖出待办 → 写进 \`kb_root/memory/briefing/<今天>.json\`。
4. 读一下脚本输出，用一句话向我汇报：命中几条微信待办、分别是回复谁。提醒我去**对话框/自动化页顶部的晨报卡片**查看，点「让我去做」我会帮你拟回复。
   - 若脚本提示 master key 失效（我多半重新登录过微信）→ 转告我需要重新跑一次 \`wx_setup.py\` 授权。

注意：这条流程产出的是「待办清单」，不是替我回消息——任何真正的回复都由我在晨报卡片里点确认后才发生。`;

function seedFlowsV4(): AutomationFlow[] {
  const now = Date.now();
  return [
    {
      projectId: null,
      ...
      color: FLOW_COLORS[3],
      description:
        "每天本地解密微信 → 挖「你回过话的私聊 + 活跃群里最近 7 天别人发来你还没回」的消息 → 写进晨报卡片，点一下帮你拟回复。需先做一次性授权。全本地不发布",
      prompt: wechatTasksPrompt,
...
export const useAutomationStore = defineStore("automation", () => {
  ...
  function load() {
    ...
    // v4 增补「微信聊天 · 每日待办」：放最前，对老用户也补一次，删除后不回种
    if (!localStorage.getItem(SEED_V4_KEY)) {
      flows.value = [...seedFlowsV4(), ...flows.value];
      localStorage.setItem(SEED_V4_KEY, "1");
      persistNow();
    }
  }
  ...
  // 关键写入(种入内置流程 / 增删流程)走同步落盘:否则 debounce 窗口内崩溃会丢掉刚种入的
  // 内置流程标记与数据 →「删除后不回种」。清掉待触发的 debounce,立即序列化一次。
  function persistNow() {
    clearTimeout(persistTimer);
    try {
  ...
  function saveFlow(draft: FlowDraft): AutomationFlow {
    ...
    if (draft.id) {
      ...
      if (i >= 0) {
        ...
        persistNow(); // 关键编辑同步落盘,避免 debounce 窗口崩溃丢失本次改动
  ...
  function removeFlow(id: string) {
    ...
    persistNow(); // 关键删除同步落盘,避免 debounce 窗口崩溃后「删了又回来 / 该删的没删」
  }
  ...
  async function runFlow(f: AutomationFlow): Promise<string | null> {
    ...
    if (!projectId) {
      // 曾经静默返回 null →「运行」像是没反应。明确报错让用户知道要先选/指定项目。
      const { toast } = await import("../composables/useToast");
      toast.error("运行失败:请先在侧栏选择一个项目，或在该自动化里指定运行项目");
      return null;
    }

    // 在任何 await 之前先把本轮 lastRunAt 打点(占位运行态):否则首轮建会话/发送要跨越
    // 多个 await,期间 60s ticker 会因「还没落 lastRunAt / lastConvId」而重复触发同一 daily 流程。
    const startIdx = flows.value.findIndex((x) => x.id === f.id);
    if (startIdx >= 0) {
      flows.value[startIdx] = { ...flows.value[startIdx], lastRunAt: Date.now() };
      flows.value = [...flows.value];
      persist();
    }
  ...
  function tick() {
    ...
    for (const f of flows.value) {
      ...
      if (s.kind === "daily" && s.time) {
        ...
        // 首次见到这条 daily 流程(从无 lastRunAt): 不要仅因「今天的点已过」就凭空补跑一次
        // (否则 09:00 后一开自动化页就自发起一次 Claude 运行)。只登记基线 lastRunAt=now:
        //  · 若此刻早于今天的点 → 稍后 ticker 越过该点(before→after 跳变)时才触发, 是真正的到点;
        //  · 若此刻已过今天的点 → now>=scheduled 但 lastRunAt(now)>=scheduled, 今天不跑, 明天再触发。
        // 除非该流程显式 runImmediately 才在首次到点补跑。
        if (f.lastRunAt == null) {
          if (!(f.runImmediately && now.getTime() >= scheduled)) {
            const i = flows.value.findIndex((x) => x.id === f.id);
            if (i >= 0) {
              flows.value[i] = { ...flows.value[i], lastRunAt: now.getTime() };
              flows.value = [...flows.value];
              persist();
            }
            continue;
